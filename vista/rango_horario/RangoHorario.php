...
<script>
    Phx.vista.RangoHorario=Ext.extend(Phx.gridInterfaz,{
            ...
            constructor:function(config){
                ...
                this.setColumnHeader('viernes',String.format('<div style="background-color: {0};"> {1}</div>','#f9d755', 'Viernes'));
                this.setColumnHeader('sabado',String.format('<div style="background-color: {0};"> {1}</div>','#7e80f9', 'Sabado'));
                this.setColumnHeader('domingo',String.format('<div style="background-color: {0};"> {1}</div>','#f9655e', 'Domingo'));
                this.setColumnHeader('hora_entrada',String.format('<div style="background-color: {0}; "> {1}</div>','#81f98f', 'Hora Entrada'));
                this.setColumnHeader('hora_salida',String.format('<div style="background-color: {0};"> {1}</div>','#f9c8ac', 'Hora Salida'));
                ...
                this.addButton('RegistrarLicencia', {
                    ...
                    iconCls: 'blist',
                    disabled: false,
                    handler: this.onGrupoTrabajo,
                    tooltip: '<b>Crea grupos especiales por funcionarios</b>'
                });
            ...
            Atributos:[
                ...
                {/// revisar
                    ...
                    id_grupo:3,
                    valorInicial: 'no',
                    filters: { pfiltro: 'rho.domingo', type: 'string' },
                    grid: true,
                    form: true
                },/// revisar
                {
                ...
                },
                {
                    config:{
                        ...
                        format: 'H:i:s',
                        renderer:function (value,p,record){
                            return String.format('<b>{0}</b>', value?value.dateFormat('H:i:s'):'');
                        }
                    },
                    type:'TimeField',
            ...
            fields: [
                ...
                {name:'hora_entrada', type: 'date',dateFormat:'H:i:s'},
                {name:'hora_salida', type: 'date',dateFormat:'H:i:s'},
                {name:'rango_entrada_ini', type: 'date',dateFormat:'H:i:s'},
                {name:'rango_entrada_fin', type: 'date',dateFormat:'H:i:s'},
                {name:'rango_salida_ini', type: 'date',dateFormat:'H:i:s'},
                {name:'rango_salida_fin', type: 'date',dateFormat:'H:i:s'},
            ...
            oncellclick : function(grid, rowIndex, columnIndex, e) {/// revisar
                ...
                if (fieldName === 'lunes' || fieldName === 'martes'|| fieldName === 'miercoles' ||
                    fieldName === 'jueves' || fieldName === 'viernes'|| fieldName === 'sabado'|| fieldName === 'domingo')
            ...
            onGrupoTrabajo: function () {
                var data = {};
                Phx.CP.loadWindows('../../../sis_asistencia/vista/grupo_asig/GrupoAsig.php',
                    'Grupos de trabajo',
                    {
                        modal: true,
                        width: '50%',
                        height: '90%'
                    }, data, this.idContenedor, 'GrupoAsig')
            },
            tabsouth:[
                {
