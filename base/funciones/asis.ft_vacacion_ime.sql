          ...
          --Obtenemos la gestion
           ...
           SELECT
            ...
            FROM wf.f_inicia_tramite(
               ...
               ---  raise exception 'llega hhh %  afas %',v_codigo_proceso,v_id_macro_proceso;
            ...
            v_codigo_estado,
            v_nro_tramite,
            v_parametros.medio_dia)RETURNING id_vacacion into v_id_vacacion;
	...
	***********************************/
        ...
        begin
            ...
            IF v_cant_dias = 0 then -- contador de dias
	            ...
	            --RAISE EXCEPTION 'ERROR: DIA NO PERMITIDO.';
            END IF;
         --RAISE EXCEPTION 'medios dias: %', v_parametros.medios_dias;
            IF(v_parametros.medios_dias = TRUE)THEN
            	v_cant_dias = v_cant_dias/2;
            END IF;
            --raise exception 'entra %',v_cant_dias;
            /*
            IF v_cant_dias < v_parametros.dias::numeric AND v_parametros.dias::numeric >= 0.5 then
            	v_mensaje = 'ERROR: CANTIDAD DE DIAS MAXIMO PERMITIDO: '|| v_cant_dias ;
	            --RAISE EXCEPTION 'ERROR: CANTIDAD DE DIAS MAXIMO PERMITIDO: %', v_cant_dias ;
            END IF;

            --v_parte_decimal = SELECT split_part(v_parametros.dias, '.', 2);
            IF((SELECT split_part(v_parametros.dias::varchar, '.', 2))::varchar != '5')THEN
            	v_mensaje = 'ERROR, DECIMAL NO PERMITIDO!'||v_parte_decimal;
            	--RAISE EXCEPTION 'ERROR, DECIMAL NO PERMITIDO! %', v_parte_decimal ;
            END IF;
              */
			--Definicion de la respuesta
